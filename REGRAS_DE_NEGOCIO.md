...
4. `valor_proposta_calculado` (proposta inicial — fallback para Concorrências fechadas, v2.0.9)
...
### 6.3 Situação Fonte - Comprasnet

**Via Siasg**: Campo `D_CMPR_SIT_ATUAL_COMPRA.ID_CMPR_SIT_ATUAL_COMPRA`:
...
**Via Comprasnet**: Campo `tbl_pregao.prgStatus` (apenas para pregões - modalidade 5):
...
Quando houver censura, mostrar intervalos ao invés do valor exato:
...
O campo `item.orcamento_sigiloso = 'S'` apenas **indica** que o item teve orçamento sigiloso, mas o valor está disponível normalmente no campo `valor_estimado` (também recomenda-se **verificar esta regra periodicamente**, haja vista que a CGPLA às vezes altera sem aviso prévio e há planos para separar os valores sigilosos do FaseExterna também no db_compras_sigiloso).
...
| `vw_licitacoes_faseexterna_sc` | Licitações do FaseExterna | db_compras.ComprasGov_FaseExterna |
| `vw_licitacoes_comprasnet_sc` | Licitações do Comprasnet | db_compras.comprasnet |
...
| `vw_itens_faseexterna_sc` | Itens do FaseExterna | db_compras.ComprasGov_FaseExterna |
| `vw_itens_siasg_sc` | **Itens do Siasg DW (segunda fonte de itens)** | db_dwsiasg |
| `vw_itens_comprasnet_sc` | Itens do Comprasnet | db_compras.comprasnet e db_compras_sigiloso.comprasnet |
| `vw_itens_consolidada_sc` | **View principal de itens** (4 fontes) | FaseExterna → Siasg → PNCP → Comprasnet |
| `sp_itens_licitacoes_pbi` | **Stored Procedure para Power BI** - itens com censura de sigilo | vw_itens_consolidada_sc + vw_licitacoes_consolidada_sc +  db_cnpj.dbo.CNPJ (dados de fornecedores)|
...
**Última verificação:** Abril/2026 - Resultado: vazio (todos os registros idênticos)
...
**Correção v2.0.9:** Adicionar `id DESC` como critério de desempate em todos os `ROW_NUMBER`:
...
- A `vw_itens_consolidada_sc` faz anti-join em nível de **licitação** — se a licitação existe no FaseExterna, os itens do Comprasnet/Siasg são excluídos, mesmo que o FaseExterna não tenha itens.
...
**Correção v2.0.8 — Comprasnet:** Valores sentinela do `codGrupo` (0, -1, -2, -3) eram exibidos como números de grupo válidos. Corrigido para tratar ≤ 0 como NULL:
...
**Decisão v2.0.8:** O prazo é calculado na **stored procedure** (`sp_itens_licitacoes_pbi`), usando as datas da view de **licitações** consolidada — não da view de itens.
...
#### 17.2.6 Normalização de Critério de Julgamento e Critério de Valor (v2.0.10)
...
#### 17.2.7 Campos Removidos da SP (v2.0.10)
...
#### 17.3.4 Enriquecimento de Dados do Fornecedor via `db_cnpj` (v2.0.9, atualizado v2.0.10)
...
>
> **Performance (v2.0.10):** O enriquecimento é feito via tabela temporária (#temp_cnpj) em 3 etapas: (1) coleta CNPJs distintos, (2) busca dados com JOINs nas tabelas de referência, (3) JOIN no SELECT final. Evita JOIN direto com `db_cnpj.dbo.CNPJ` (tabela grande) no SELECT principal.
...
> **⚠️ ATENÇÃO (v2.0.8):** `ID_ITCP_MATERIAL_SERVICO` **NÃO é chave única** na `D_ITCP_MATERIAL_SERVICO`. O mesmo código pode existir em duas linhas — uma como Material (`ID_ITCP_GRUPO_MATERIAL > 0`) e outra como Serviço (`ID_ITCP_GRUPO_SERVICO > 0`). Ver §17.4.5 para a regra de desambiguação.
...
| **FaseExterna** | `TRY_CAST(codigo_item_catalogo AS INT)` | Precisa filtrar por `tipo_item_catalogo` ('M'/'S') | ✅ **Corrigido v2.0.8** |
...
**Correção no FaseExterna (v2.0.8):**
...
#### 17.5.1 Segurança de Tipos em UNION ALL (Lição v2.0.8)
...
**Conflitos encontrados e corrigidos na v2.0.8:**
...
A censura nos itens é aplicada no **nível de licitação** (não de item individual):

**Regra:** Quando `Situacao_Gerada = 'Não Homologada'` E `qtd_itens_sigilosos > 0`:
→ Zerar TODOS os valores de TODOS os itens daquela licitação, independente do item individual ser sigiloso ou não (pois poderia causar inferências, a depender da licitação).
...
| Abril/2026 | 2.0.10 | **Link PNCP na `Consulta_LicitacoesSC_PBI`** — URL usa `sequencialcompra` (não `numerocompra`). JOIN com `orgao` para obter CNPJ (§16.7) |
| Abril/2026 | 2.0.10 | Documentação: §16.1 — PNCP exclusivo da Lei 14.133/2021, numeração sequencial por CNPJ+ano, sem colisão de modalidades |
| Abril/2026 | 2.0.10 | Melhoria: `Natureza Jur. Fornecedor` agora traz descrição via JOIN com `db_cnpj.dbo.NaturezaJuridica` (§17.3.4) |
| Abril/2026 | 2.0.10 | Melhoria: `Tipo Natureza Jur. Fornecedor` — campo novo via `db_cnpj.dbo.TipoNaturezaJuridica` (ex: "ENTIDADES EMPRESARIAIS") |
| Abril/2026 | 2.0.10 | Melhoria: `Porte Fornecedor` mapeado para descrição (ME/EPP/Demais) via CASE na temp table (§17.3.4) |
| Abril/2026 | 2.0.10 | Documentação: §17.2.6 — mapeamento completo de `criterio_julgamento_item` e `criterio_valor` (2 codificações do FaseExterna + Comprasnet + Siasg) |
